...
constexpr const char *const SURNAME = "Budziak";
constexpr const char *const MAIL = "budziakszymo@student.agh.edu.pl";
constexpr const char *const BOOK_ID = "405992";
...
compileTimeStrlen(const char *text)

noexcept
{
return (*text ? 1+
compileTimeStrlen(text
+1) : 0);
}

constexpr inline size_t

compileTimeCountFirstDigits(const char *text)
...
text &&*text
<='9') ? 1+
compileTimeCountFirstDigits(text
+1) : 0);
}

constexpr inline bool compileTimeIsDigit(const char *text)
...
constexpr inline bool compileTimeContains(const char *text, char letter)
...
return *text == letter ? 1 :
compileTimeContains(text
+1, letter);
}
